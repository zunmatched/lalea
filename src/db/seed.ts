...
const exercises = [
  ...
  { id:"10000000-0000-4000-8000-000000000003", position:3, type:"reading_choice", prompt:"請對方帶你完整說明流程，選出最適合的用詞。", content:{ context:"Could you ______ me through the process?", options:[{id:"walk",text:"walk"},{id:"hold",text:"hold"},{id:"push",text:"push"}]}, answer:{correctIds:["walk"]}, feedback:{correct:"walk someone through something 表示帶對方一步步說明，是很自然的說法。",incorrect:"正確用法是 walk me through the process，walk someone through 才表示帶對方一步步說明。"}},
...
for (const exercise of exercises) await db.insert(s.exercises).values({ ...exercise, learningUnitId: ids.unit }).onConflictDoUpdate({ target: s.exercises.id, set: { type: exercise.type, prompt: exercise.prompt, content: exercise.content, answer: exercise.answer, feedback: exercise.feedback } });
...
const extraExercises=[
 ...
 {unitId:extraCourses[0].unitId,id:"10000000-0000-4000-8000-000000000022",position:3,type:"reading_choice",prompt:"完成明確的更新承諾，選出最適合的介系詞。",content:{context:"I'll send you an update ______ noon.",options:[{id:"by",text:"by"},{id:"until",text:"until"},{id:"since",text:"since"}]},answer:{correctIds:["by"]},feedback:{correct:"by noon 表示最晚在中午前完成，清楚承諾時間點。",incorrect:"正確用法是 by noon，by 表示「最晚在...之前」，until/since 在這裡不適用。"}},
 ...
 {unitId:extraCourses[1].unitId,id:"10000000-0000-4000-8000-000000000032",position:3,type:"reading_choice",prompt:"禮貌地提議改期，選出最適合的動詞。",content:{context:"Could we ______ it to Friday morning?",options:[{id:"move",text:"move"},{id:"hold",text:"hold"},{id:"leave",text:"leave"}]},answer:{correctIds:["move"]},feedback:{correct:"move it to Friday morning 是常見的改期用法。",incorrect:"正確用法是 move it to Friday morning，move 才有「更改時間/地點」的意思。"}},
...
for(const exercise of extraExercises)await db.insert(s.exercises).values({id:exercise.id,learningUnitId:exercise.unitId,position:exercise.position,type:exercise.type,prompt:exercise.prompt,content:exercise.content,answer:exercise.answer,feedback:exercise.feedback}).onConflictDoUpdate({target:s.exercises.id,set:{type:exercise.type,prompt:exercise.prompt,content:exercise.content,answer:exercise.answer,feedback:exercise.feedback}});
const audioDrafts=[
