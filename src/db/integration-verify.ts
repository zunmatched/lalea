...
async function main() {
  ...
  const review=(dimension:"reading_recognition"|"listening_recognition"|"active_recall",clientEventId:string,isCorrect:boolean)=>submitReview(new Request("http://test/api/reviews",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({userVocabularyId:personalWord.id,dimension,clientEventId,isCorrect})}));
