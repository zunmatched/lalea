...
async function main() {
  ...
  await adminDb.update(s.vocabularyMasteryStates).set({nextReviewAt:new Date(0)}).where(eq(s.vocabularyMasteryStates.id,readingState.id));const queueBody=await (await getReviewQueue(new Request("http://test/api/reviews/queue"))).json();assert.equal(queueBody.due[0].dimension,"reading_recognition");assert.equal(queueBody.policy.dueFirst,true);
