...
async function main() {
  ...
  const exerciseRows=[
    ...
    {id:"10000000-0000-4000-8000-000000000103",position:3,type:"branched_dialogue"},
  ];
  ...
  const review=(dimension:"reading_recognition"|"listening_recognition"|"active_recall",clientEventId:string,isCorrect:boolean,challengeType:"spell"|"dictation"="spell")=>submitReview(new Request("http://test/api/reviews",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({userVocabularyId:personalWord.id,dimension,challengeType,clientEventId,isCorrect})}));
  const reviewEvent="50000000-0000-4000-8000-000000000101";const firstReview=await review("reading_recognition",reviewEvent,false);assert.equal((await firstReview.json()).proficiency,0,"a wrong answer scores zero for the day");
  const duplicateReview=await review("reading_recognition",reviewEvent,true);const duplicateBody=await duplicateReview.json();assert.equal(duplicateBody.idempotent,true);assert.equal(duplicateBody.proficiency,0,"duplicate clientEventId must not reprocess with the new isCorrect value");
  await review("listening_recognition","50000000-0000-4000-8000-000000000102",true);await review("active_recall","50000000-0000-4000-8000-000000000103",true);
  ...
  const readingState=vocabularyStates.find(state=>state.dimension==="reading_recognition")!;assert.equal(readingState.reviewCount,1,"a deduplicated retry must not double-count reviewCount");const readingEvents=await adminDb.select().from(s.reviewEvents).where(eq(s.reviewEvents.vocabularyMasteryStateId,readingState.id));const[up1Row]=await adminDb.select({reviewWindowDays:s.userLearningPaths.reviewWindowDays}).from(s.userLearningPaths).where(eq(s.userLearningPaths.id,ids.up1));const replayedProficiency=computeProficiency(readingEvents.map(event=>({isCorrect:event.isCorrect,createdAt:event.createdAt,challengeType:event.challengeType as "spell"|"dictation"})),up1Row.reviewWindowDays);const storedProficiency=(readingEvents[readingEvents.length-1].afterState as{proficiency:number}).proficiency;assert.equal(replayedProficiency,storedProficiency,"immutable events must rebuild the derived proficiency");
  const queueBody=await (await getReviewQueue(new Request("http://test/api/reviews/queue"))).json();const dueDimensions=queueBody.due.map((item:{dimension:string})=>item.dimension);assert.deepEqual([...dueDimensions].sort(),["active_recall","listening_recognition","reading_recognition"],"due queue must include every studied dimension, mastered or not");assert.equal(queueBody.policy.dueFirst,true);
