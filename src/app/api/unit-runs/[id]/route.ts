...
export async function GET(_:Request, context:{params:Promise<{id:string}>}) {
  ...
  const assets=await db.select().from(audioAssets).where(eq(audioAssets.learningUnitId,run.unitId));const assetByExercise=new Map(assets.map(asset=>[asset.exerciseId,asset]));
