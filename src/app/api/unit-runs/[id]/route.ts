...
import { exercises, unitRuns, userLearningPaths } from "@/db/schema";
...
export async function GET(_:Request, context:{params:Promise<{id:string}>}) {
  ...
  const payload=rows.map((exercise,index)=>{const content=exercise.content as {options?:Array<{id:string;text:string}>;[key:string]:unknown}; return {id:exercise.id,position:exercise.position,type:exercise.type,prompt:exercise.prompt,content:{...content,options:content.options?seededShuffle(content.options,run.seed+index):undefined}}});
