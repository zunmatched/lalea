...
export async function POST(request:Request){
 ...
 const inserted=await db.transaction(async tx=>{const rows=await tx.insert(reviewEvents).values({clientEventId:data.clientEventId,vocabularyMasteryStateId:state.id,isCorrect:data.isCorrect,rating:data.rating,beforeState:before,afterState:after,reason:after.reason,schedulerVersion:after.schedulerVersion}).onConflictDoNothing().returning({id:reviewEvents.id});if(!rows.length)return false;await tx.update(vocabularyMasteryStates).set({intervalDays:after.intervalDays,nextReviewAt:after.nextReviewAt,lastReviewedAt:now,reviewCount:after.reviewCount,schedulerVersion:after.schedulerVersion}).where(eq(vocabularyMasteryStates.id,state.id));await tx.update(userVocabulary).set({status:data.isCorrect?"learned":"learning",firstLearnedAt:owned.firstLearnedAt??now}).where(eq(userVocabulary.id,owned.id));return true});
