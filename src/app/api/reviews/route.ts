...
const input=z.object({userVocabularyId:z.string().uuid(),dimension:z.enum(["reading_recognition","listening_recognition","active_recall"]),clientEventId:z.string().uuid(),isCorrect:z.boolean()});
...
export async function POST(request:Request){
 ...
 const now=new Date();
 const priorEvents=await db.select({isCorrect:reviewEvents.isCorrect,createdAt:reviewEvents.createdAt}).from(reviewEvents).where(eq(reviewEvents.vocabularyMasteryStateId,state.id));
 const before={proficiency:computeProficiency(priorEvents,now)};
 const after={proficiency:computeProficiency([...priorEvents,{isCorrect:data.isCorrect,createdAt:now}],now)};

 const inserted=await db.transaction(async tx=>{
  const rows=await tx.insert(reviewEvents).values({clientEventId:data.clientEventId,vocabularyMasteryStateId:state.id,isCorrect:data.isCorrect,beforeState:before,afterState:after,reason:data.isCorrect?"correct":"incorrect",schedulerVersion:"day-bucket-v1"}).onConflictDoNothing().returning({id:reviewEvents.id});
  if(!rows.length)return false;
  await tx.update(vocabularyMasteryStates).set({lastReviewedAt:now,reviewCount:state.reviewCount+1,schedulerVersion:"day-bucket-v1"}).where(eq(vocabularyMasteryStates.id,state.id));
  await tx.update(userVocabulary).set({status:data.isCorrect?"learned":"learning",firstLearnedAt:owned.firstLearnedAt??now}).where(eq(userVocabulary.id,owned.id));
  return true;
 });
