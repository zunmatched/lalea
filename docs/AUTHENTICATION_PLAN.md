...
> 狀態：**暫緩，不在目前開發範圍**（2026-08-24 決策）  
> 最後更新：2026-08-21  
> 決策：Passkey 為主要登入方式，一次性恢復碼為備援
>
> **2026-08-24 更新**：確定不對外公開部署，僅透過 Tailscale 私人網路個人使用。`DEV_AUTH_ENABLED` 的開發用免登入捷徑視為目前的正式使用方式，Passkey 需要的固定公開網域與 HTTPS 也不會建置。本文件保留作為未來若改變主意（要多使用者、要公開存取）時的設計參考，不代表待辦事項。
